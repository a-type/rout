import { LongGameError, id, } from '@long-game/common';
import { RpcTarget } from 'cloudflare:workers';
import { jsonObjectFrom, sql } from '../kysely/index.js';
export class UserStore extends RpcTarget {
    ...
    async updateMe({ displayName, color, imageUrl, sendEmailUpdates, }) {
        return this.#db
            .updateTable('User')
            .set({
            displayName,
     ...
     */
    async getFriendshipInvites({ direction, }) {
        const builder = this.#db
            ...
            direction === 'incoming'
                ? (eb) => jsonObjectFrom(eb
                    ...
                    .whereRef('User.id', '=', 'FriendshipInvitation.inviterId')).as('otherUser')
                : sql `NULL`.as('otherUser'),
        ])
    ...
    async insertFriendshipInvite(invitedEmail) {
    ...
    async respondToFriendshipInvite(friendshipId, status) {
        const friendship = await this.#db
            .selectFrom('FriendshipInvitation')
            ...
            .where('User.id', '=', this.#userId)
            .executeTakeFirstOrThrow(() => new LongGameError(LongGameError.Code.NotFound, 'Friendship Invite not found'));
        if (friendship.status !== 'pending') {
            throw new LongGameError(LongGameError.Code.BadRequest, 'Friendship is not pending');
        }
        if (new Date(friendship.expiresAt) < new Date()) {
            throw new LongGameError(LongGameError.Code.BadRequest, 'Friendship invite has expired');
        }
        if (status === 'retracted') {
            // deletes the invite -- but only if it was created by the user
            if (friendship.inviterId !== this.#userId) {
                throw new LongGameError(LongGameError.Code.Forbidden, 'You cannot retract this invite');
            }
            await this.#db
    ...
    async insertFoundingGameMembership(gameSessionId) {
        ...
        if (existing.length > 0) {
            throw new LongGameError(LongGameError.Code.Forbidden, 'Game session already has members. Ask for an invite.');
        }
    ...
    #encodeCursor = (val) => btoa(val);
    #decodeCursor = (val) => atob(val);
    async getGameSessions(filter = {
        first: 10,
    }) {
        let builder = this.#db
            .selectFrom('GameSessionInvitation')
        ...
        if (filter.status) {
            builder = builder.where('GameSessionInvitation.status', '=', filter.status);
        }
        if (filter.first) {
     ...
     */
    async getGameSessionMembers(gameSessionId) {
        ...
        return users.map((u) => {
            return {
                ...u,
                color: (u.color === null ? 'gray' : u.color),
            };
        });
    }
    async getGameSessionInvitations(status) {
        return this.#db
            .selectFrom('GameSessionInvitation')
    ...
    async getGameSessionInvitationForSpecificSession(sessionId) {
        return this.#db
            .selectFrom('GameSessionInvitation')
    ...
    async sendGameSessionInvitation(gameSessionId, userId) {
        const ownMembership = await this.#db
            ...
            .executeTakeFirst();
        if (!ownMembership) {
            throw new LongGameError(LongGameError.Code.Forbidden, 'Only game session members can invite others');
        }
    ...
    async respondToGameSessionInvitation(inviteId, response) {
        const membership = await this.#db
            .selectFrom('GameSessionInvitation')
            .where('id', '=', inviteId)
            .selectAll()
        ...
        if (membership.userId !== this.#userId) {
            throw new LongGameError(LongGameError.Code.Forbidden, 'You are not the invited user');
        }
        if (membership.status !== 'pending') {
            throw new LongGameError(LongGameError.Code.BadRequest, 'Invite is not pending');
        }
        const updated = await this.#db
     ...
     */
    async getInvitationsToGameSession(gameSessionId) {
        ...
        const membership = await this.#db
            ...
            .executeTakeFirst();
        if (!membership) {
            throw new LongGameError(LongGameError.Code.Forbidden, 'You are not a member of this game session');
        }
